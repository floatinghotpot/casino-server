...
LoginServer.prototype.startInstance = function( instance_id ) {
	...
	httpserver.listen(conf.server.port, function() {
...
LoginServer.prototype.tick = function() {
	...
	for(var i in dropped) {
		if(dropped[i] -- <= 0) {
			var gamer = gamers[i];
			if(gamer) {
				service.gamerLogout({seq:0, uid:gamer.uid, pin:gamer.pin});
				delete dropped[i];
			}
		}
	}
...
LoginServer.prototype.gamerLogin = function( socket, args ) {
	...
	db.hgetall(uid_key, function(err,userinfo){
		...
		var now = new Date().getTime();
		var gamer = new Gamer().setProfile( userinfo ).setLink( server, socket, socket.id + now );
		
		if(gamer.uid in server.gamers) {
			
		} else {
			server.gamers_count ++;
		}
		
		server.gamers[ gamer.uid ] = gamer;
