...
LoginServer.prototype.onConnected = function( socket ) {
	...
	socket.on('rpc', function(req){
		...
		var method = service[ func_name ];
		if(typeof method === 'function') {
			method.call(service, socket, req);
			
		} else {
			var gamer = socket.gamers[ req.uid ];
			if(gamer.pin !== req.pin) {
				socket.emit('rpc_ret', { seq:req.seq, err:403, ret:'invalid pin, need login first'});
				return;
			}
			
			method = gamer[ func_name ];
			if(typeof method === 'function') {
				method.call(gamer, req.args, function(err, ret){
				...
				if(roomid) {
					delete req.pin;
					service.pub.publish('room:#'+roomid, JSON.stringify(req));
				} else {
...
LoginServer.prototype.onGamer_signup = function(socket, req) {
	...
	if(uid && (typeof uid === 'string') && (uid.length>0)) {
		...
		db.incr('user:seq', function(err,ret){
			...
			service.onGamer_signup2(socket, req);
		});
	}
};

LoginServer.prototype.onGamer_signup2 = function(socket, req) {
	var args = req.args;
	var uid = args.uid;
	var db = this.db;
	...
	db.hgetall(uid_key, function(err,ret){
		if(err) { socket.emit('rpc_ret', { seq: req.seq, err:500, ret: 'db err' }); return; }
		if(ret) {
			socket.emit('rpc_ret', { seq: req.seq, err:409, ret: 'login id ' + loginid + ' exists' });
			return;
		}
		...
		db.multi()
			...
			.exec(function(err,ret){
				if(err) { socket.emit('rpc_ret', { seq: req.seq, err:500, ret: 'db err' }); return; }
				socket.emit('rpc_ret', { seq: req.seq, err:0, ret:{ uid:uid, passwd: args.passwd } });
			});
	});
};
...
LoginServer.prototype.onGamer_logout = function( socket, req ) {
	...
	if(gamer && (req.pin == gamer.pin)){
		if(gamer.room) {
			gamer.onGamer_exit(0, function(err, ret){
		...
		socket.emit('rpc_ret', { seq:req.seq, err:0, ret:'ok' });
	} else {
