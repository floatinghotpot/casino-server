...
HoldemGame.prototype.gameOver = function() {
	...
	for(i=0; i<in_gamers.length; i++) {
		...
		scorelist.push(item);
		console.log(item.seat, item.cards, item.chips, item.prize);
	}
	console.log(room.shared_cards, room.pot, room.cards, room.ingamers_count);
...
HoldemGame.prototype.gamerMoveTurn = function(move) {
	...
	console.log('gamer move turn ', room.ingamers_count);
...
HoldemGame.prototype.gamerGiveUp = function( gamer ) {
	...
	if(room.ingamers_count === 1) {
		room.simpleWin();
		console.log('we have simple win');
...
HoldemGame.prototype.gamerShowDown = function() {
	...
	for(i=0; i<in_gamers.length; i++) {
		...
		if(maxFive) {
			...
			finals.push( gamer );
			console.log(gamer.uid, gamer.seat, gamer.maxFiveRank);
		}
	}
	...
	if(someone_allin) {
		...
		var prize = room.pot;
		room.evaluate_pot(finals, prize);
	}
...
HoldemGame.prototype.evaluate_pot = function(finals, prize) {
	 ...
	 * of the prize. Prize value is adapted in finals.
	 */
		var n = finals.length;
		if(n > 1) {
