...
Room.prototype.onGamer_exit = function(req, reply) {
	var room = this;
	var uid = req.uid;
	var gamers = this.gamers;
	...
	for(var i in exitcmds) cmds[i] = exitcmds[i];

	reply(0, {
		cmds: cmds
	});
	
	room.pub.publish('user:log', 'gamer (' + uid + ') exit room #' + room.id );
};
...
Room.prototype.onGamer_takeseat = function(req, reply) {
	...
	var gamers = this.gamers;
	var gamer = gamers[ uid ];
	console.log(gamer.profile.coins);
