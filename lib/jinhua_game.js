...
JinhuaGame.prototype.gameStart = function() {
	...
	var seat;
	var chip_base = room.options.chip_base;
	for(j=0, len=in_gamers.length; j<len; j++) {
		...
		gamer.profile.coins -= chip_base;
		room.chip_total += chip_base;
		
		ingame_seats.push( seat );
		
		roomcards[ seat ] = Jinhua.sort( Poker.draw(fullcards, 3) );
		roomchips[ seat ] = chip_base;
...
JinhuaGame.prototype.gameOver = function() {
	...
	if(in_gamers.length === 1) {
		...
		var prize = Math.round( room.chip_total * (1 - room.options.rake));
...
JinhuaGame.prototype.gamerMoveTurn = function(move) {
	...
	var add_options = [];
	if(room.options.addchip_multiple) {
		for(i=1; i<=3; i++) {
			add = room.chip_min + room.chip_min * i;
			if(chip_max > 0) {
		...
		for(i=1; i<=3; i++) {
			add = room.chip_min + room.options.addchip_min * i;
			if(chip_max > 0) {
				if(add > chip_max) continue;
			}
			add_options.push( add );
...
JinhuaGame.prototype.gamerLose = function(loser) {
	...
	var in_gamers = room.in_gamers;
	for(var i=0, len=in_gamers.length; i<len; i++) {
		if(in_gamers[i].seat === loser.seat) {
...
JinhuaGame.prototype.onGamer_giveup = function(req, reply) {
...
JinhuaGame.prototype.onGamer_follow = function(req, reply) {
	var room = this, uid = req.uid;
	var gamers = room.gamers;
	var gamer = gamers[ uid ];
...
JinhuaGame.prototype.onGamer_addchip = function(req, reply) {
	var room = this, uid = req.uid;
	var gamers = room.gamers;
	var gamer = gamers[ uid ];
	if(gamer.is_ingame) {
		...
		var chip_max = room.options.chip_max;
		if(isNaN(add) || (add < room.chip_min) || ((chip_max > 0) && (add > chip_max))) {
			reply(400, 'invalid chip to add: ' + n);
			return;
		}
		var n = (gamer.is_cardchecked) ? (add * 2) : add;
		if(n <= gamer.profile.coins) {
			room.chip_min = add;
			...
			reply(400, 'no enough coins to add: ' + n);
		}
...
JinhuaGame.prototype.onGamer_checkcard = function(req, reply) {
	var room = this, uid = req.uid;
	var gamers = room.gamers;
	var gamer = gamers[ uid ];
	if(gamer.is_ingame) {
